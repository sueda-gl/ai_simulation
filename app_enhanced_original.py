...
# Helper function to read decision parameters directly
def get_decision_global_parameters(selected_decisions: List[str]) -> set:
    ...
    try:
        decisions_path = Path(__file__).resolve().parents[0] / "config" / "decisions.yaml"
...
def get_all_global_parameters() -> set:
    ...
    try:
        simulation_path = Path(__file__).resolve().parents[0] / "config" / "simulation.yaml"
