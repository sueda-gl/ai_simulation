...
GROUP_COL   = "Group"                   # group assignment
...
exper_keep  = [c for c in [ID_COL, DV_COL, PROG_COL, GROUP_COL] if c in exper.columns]
...
df["group"] = pd.Categorical(df["Group"], categories=["Base", "MidSub", "NoSub", "HighSub"], ordered=True)
...
model = smf.ols("obs_behav ~ z_hh + C(income_cat) + C(study_prog) + C(group)", data=df).fit()
...
df["observed_behaviour_scaled"]   = to0100(df["obs_behav"],   y_min, y_max)
o_min, o_max     = float(df["other_score"].min()), float(df["other_score"].max())
df["predicted_behaviour_scaled"] = to0100(df["other_score"], o_min, o_max)

# ========== 6) Anchors ==========
for w in WEIGHTS:
    df[f"anchor_{int(w*100)}"] = w*df["observed_behaviour_scaled"] + (1-w)*df["predicted_behaviour_scaled"]
...
# ========== 8) Save outputs ==========
out_cols = [ID_COL, "obs_behav", "observed_behaviour_scaled", "predicted_behaviour_scaled"] + \
           [f"anchor_{int(w*100)}" for w in WEIGHTS] + \
           [f"pred_{int(w*100)}" for w in WEIGHTS] + \
           ["income_eur", "income_cat", "income_draw", "z_hh", "study_prog", "group"]

df[out_cols].to_excel("phase2_outputs_with_group.xlsx", index=False)
...
with open("phase2_params_with_group.json", "w") as f:
    json.dump(params, f, indent=2)

with open("phase2_model_summary_with_group.txt", "w") as f:
    f.write(model.summary().as_text())

print("Phase 2 complete. Files written: 'phase2_outputs_with_group.xlsx', 'phase2_params_with_group.json', 'phase2_model_summary_with_group.txt'")